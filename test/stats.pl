...
while($line = <INFILE>) {
	...
	foreach my $i (1..26) { #$i loops from C1 to C26 iterating over the respective hashes
		chomp $tokens[14+$i];
		my ($imp,$click)=(1,$tokens[1]);
		$aoh[$i]{$tokens[14+$i]}=$aoh[$i]{$tokens[14+$i]}+($imp,$click); #Adding impressions for every categorical value of Ci
	}
}

foreach my $i (1..26) {
	while (my ($k,$v)=each $aoh[$i]){print "C${i}I $k $v\n"};
}
