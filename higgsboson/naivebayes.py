...
all = list(csv.reader(open("training_10.csv","rb"), delimiter=','))

# Slicing off header row and id, weight, and label columns.

xs = np.array([map(float, row[1:-2]) for row in all[1:]])
(numPoints,numFeatures) = xs.shape
print "Number of data points = %d, Number of features = %d" % (numPoints, numFeatures)
...
print len(sSelector), len(sSelectorTrain)
...
print range(0, numPointsTrain+1, numPointsTrain/numBins)
print logPs.shape
print binMaxs.shape
print binIndexes
...
# In[49]:
print "Save the submission file"
